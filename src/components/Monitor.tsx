...
export function Monitor({ isActive, onClick, theme, onOpenProject }: MonitorProps) {
  ...
  const screenContent = (
    <div
      className={`w-full h-full relative bg-[#0f111a] text-white overflow-hidden ${
        isActive || isFullscreen ? 'pointer-events-auto' : 'pointer-events-none'
      }`}
    ...
    >
      ...
      <div
        ...
        style={{ backgroundImage: `url("${config.desktop.wallpaperUrl}")` }}
      />
        ...
        <div className="grid grid-cols-4 gap-x-12 gap-y-12 max-w-4xl">
          {projects.map((project, i) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-24 h-24 rounded-3xl border flex items-center justify-center transition-all shadow-xl relative ${colorClass} hover:scale-105 hover:brightness-110 backdrop-blur-md`}
                >
                  {project.status === 'in-progress' && (
                    <div className="absolute -top-1 -right-1 w-3 h-3 bg-amber-500 rounded-full border-2 border-[#0f111a]" />
                  )}
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 h-16 bg-white/5 backdrop-blur-2xl border border-white/10 rounded-2xl px-4 flex items-center gap-4 shadow-2xl">
        ...
        <a
          ...
          className="w-12 h-12 bg-emerald-500 rounded-xl flex items-center justify-center shadow-lg cursor-pointer hover:-translate-y-1 transition-transform block"
        >
          <Mail className="w-6 h-6 text-white" />
        </a>

        <a
          href="https://linkedin.com/in/yousuf-shb"
          target="_blank"
          rel="noopener noreferrer"
          className="w-12 h-12 bg-[#0077b5] rounded-xl flex items-center justify-center shadow-lg cursor-pointer hover:-translate-y-1 transition-transform block"
        >
          ...
          >
            <div className="h-12 bg-white/5 border-b border-white/5 flex items-center px-4 justify-between">
              <div className="flex gap-2">
                <button
                  className="w-3 h-3 rounded-full bg-red-500 hover:bg-red-600 flex items-center justify-center group/close"
              ...
              <span className="text-white/80 text-sm font-medium flex items-center gap-2">
                <Code className="w-4 h-4" /> About This Site
              </span>
            ...
            <div className="p-6 text-white/80 text-sm leading-relaxed">
              <p className="mb-4">
                This portfolio is built using a custom 3D engine created with standard web technologies, without WebGL.
              </p>
              <p className="mb-4">
              ...
              <ul className="list-disc pl-5 mb-4 space-y-2">
                <li>
                  <strong>React</strong> for component architecture and state.
                </li>
                <li>
                  <strong>Tailwind CSS</strong> for styling.
                </li>
                <li>
                  <strong>Framer Motion</strong> for animations.
                </li>
                <li>
                  <strong>CSS 3D Transforms</strong> to construct the 3D environment in the DOM.
                </li>
              </ul>
              <p>
                Hardware accelerated transforms keep it lightweight while still feeling interactive.
              </p>
            </div>
          </motion.div>
        )}
  ...
  return (
    <>
      ...
      >
        <div
          className="absolute bottom-[-140px] left-1/2 -translate-x-1/2 w-32 h-40 bg-gradient-to-b from-[#e5e5e5] to-[#a3a3a3] dark:from-[#525252] dark:to-[#262626]"
          style={{ transform: 'translateZ(-40px) rotateX(5deg)' }}
        />
        <div
          className="absolute bottom-[-140px] left-1/2 -translate-x-1/2 w-48 h-3 bg-[#a3a3a3] dark:bg-[#262626] rounded-t-sm"
          style={{ transform: 'translateZ(10px)' }}
        />
        <div
          className="absolute bottom-[0px] left-1/2 -translate-x-1/2 w-24 h-24 bg-[#d4d4d4] dark:bg-[#404040] rounded-full"
          style={{ transform: 'translateZ(-45px)' }}
        />

        <div
          className={`w-full h-full rounded-xl border-[8px] border-[#111] overflow-hidden relative transition-shadow duration-500 ${
            isActive ? 'shadow-[0_20px_100px_rgba(0,0,0,0.5)] monitor-glow' : ''
          } bg-black`}
          style={{
            boxShadow: isActive
